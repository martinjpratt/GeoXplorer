    ...
    public class SharingWorldAnchorManager : WorldAnchorManager
    {
        ...
        private void RoomManagerListener_AnchorUploaded(bool successful, XString failureReason)
        {
            if (successful)
            {
                ...
                for (int i = 0; i < anchorIds.Length; i++)
                {
                    ...
                    if (CursorText != null)
                    {
                        CursorText.text = string.Format("Successfully uploaded anchor \"{0}\".", anchorIds[i]);
                        Destroy(CursorText, 3);
                    }
                }
            }
            else
            ...
            for (int i = 0; i < roomAnchorCount; i++)
            {
                ...
                if (roomAnchorId.GetString().Equals(anchorId))
                {
                    ...
                    if (downloadStarted)
                    {
                        ...
                        if (CursorText != null)
                        {
                            CursorText.text = "Found a match! Attempting to download anchor...";
                        }
                    }
                    else
                    {
        ...
        private void ImportComplete(SerializationCompletionReason status, WorldAnchorTransferBatch anchorBatch)
        {
            ...
            if (successful)
            {
                ...
                if (CursorText != null)
                {
                    CursorText.text = string.Format("Successfully imported \"{0}\" anchors.", anchorBatch.anchorCount.ToString());
                    Destroy(CursorText, 3);
                }
